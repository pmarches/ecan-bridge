...
class GatewayCANChannelConfiguration:
    ...
    def totoml(self):
        ...
        canTable['mysteryByte'].comment('Always has been seen as 0')
        ...
        canTable['mystery2'].comment('Always has been seen as 0000')
        canTable.add('connectionTimeout', self.connectionTimeout)
        canTable.add('mystery3', bytes.hex(self.mystery3))
        canTable['mystery3'].comment('Always has been seen as 0000')
...
class GatewayConfiguration:
    ...
    def totoml(self):
        ...
        doc['mystery0'].comment('Always has been seen as 00a5')
        doc.add(tomlkit.nl())
        ...
        doc.add('obtainDHCP', self.obtainDHCP)
        doc['obtainDHCP'].comment('0=Static address, 1=DHCP Enabled')
        ...
        doc['noCANDataAutoReboot'].comment("Number of seconds without CANBus traffic for device to reboot. 0 Disables, 60-65535 seconds")
...
class ProprietaryConfigFileReader:
    ...
    CONFIG_ZERO_STRUCT_FORMAT='<2s32s11sBBBBBB11s26s4s4s4s4sB129sHBBH';
    ...
    def parseConfigurationZero(configurationBytes, config):
        ...
        config.obtainDHCP=parts[15]
        config.reportTargetIp=parts[16].decode('ascii').rstrip('\x00')
        config.reportTargetPort=parts[17]
        config.reportCycle=parts[18]
        config.reconnectionTimeout=parts[19]
        config.noCANDataAutoReboot=parts[20]
...
def rebootGateway(deviceIpAddr):
    ...
    if rebootResponse!=expectedRebootResponse:
        error('Unexpected reboot response %s', binascii.hexlify(rebootResponse))
    ...
    try:
        ...
        while(True):
            if(bus.state!=can.BusState.ACTIVE):
                ...
                break
            debug('About to select %s', str(BOTH_SOCKETS))
            timeout=10.0
            readyTriplet=select.select(BOTH_SOCKETS, [], BOTH_SOCKETS, timeout)
            debug(readyTriplet)
